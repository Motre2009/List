...
internal class Program
{
    ...
    {
        var cmd = conn.CreateCommand();
        cmd.CommandText = @"SELECT b.FullName, b.Email FROM Buyers b JOIN Cities c ON b.CityId = c.CityId WHERE c.CityName = $city";
    ...
    {
        var cmd = conn.CreateCommand();
        cmd.CommandText = @"SELECT b.FullName, b.Email FROM Buyers b JOIN Countries c ON b.CountryId = c.CountryId WHERE c.CountryName = $country";
    ...
    {
        var cmd = conn.CreateCommand();
        cmd.CommandText = @"SELECT p.PromoName, p.StartDate, p.EndDate FROM Promotions p JOIN Countries c ON p.CountryId = c.CountryId WHERE c.CountryName = $country";
        cmd.Parameters.AddWithValue("$country", country);
        using var reader = cmd.ExecuteReader();
        while (reader.Read())
            Console.WriteLine($"{reader[0]} | {reader[1]} → {reader[2]}");
    ...
    static void InsertBuyerInteraction(SqliteConnection conn)
    {
        Console.Write("Ім’я: ");
        ...
        var cmd = conn.CreateCommand();
        cmd.CommandText = @"INSERT INTO Buyers (FullName, BirthDate, Gender, Email, CountryId, CityId) VALUES ($name, $birth, $gender, $email, $cid, $city)";
